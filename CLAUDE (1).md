...
Before any substantive task, read in this order (skip what doesn't exist):
1. `LESSONS.md` (repo root) — past corrections and confirmed approaches
2. The project's `.claude/skills/*/SKILL.md` — project context and checks
3. `docs/decisions/` or ADRs if present — settled decisions; do not re-propose rejected options
4. The modules named in or implicated by the request
Read the index/summary level first; open full files only when relevant.
...
## Contradiction audit
- When the user says "audit memory" (or roughly monthly if asked to check), read this CLAUDE.md, all LESSONS.md files, and all project SKILL.md files. Flag entries that contradict each other, contradict the current code, or are stale (renamed files, changed stack, reversed decisions). Present the conflicts as a list with a recommended resolution each — the user rules on them; do not delete or rewrite entries without approval.
